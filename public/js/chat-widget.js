const U="1winex-chat-v2",ee="1winex-chat-theme",te=/^(localhost|127\.0\.0\.1)$/.test(location.hostname)?"/api/chat":"https://api-chat.net/api/chat",ae="1winex",$="1winex-chat-root";function re(){return typeof document<"u"&&(document.documentElement.lang||"").toLowerCase().startsWith("ru")}function S(e,t){return re()?t:e}async function oe(e,t,a){let o;try{o=await fetch(te,{method:"POST",headers:{"Content-Type":"application/json",Accept:"text/event-stream"},body:JSON.stringify(e),signal:a})}catch(l){if(l?.name==="AbortError"){t.onDone();return}t.onError(S("Network error. Please try again.","Нет соединения. Попробуйте ещё раз."));return}if(!o.ok){let l=S("Request failed. Please try again.","Не удалось получить ответ. Попробуйте ещё раз.");try{const m=await o.json();m?.error&&(l=m.error)}catch{}t.onError(l);return}if(!o.body){t.onError(S("Empty response from server.","Сервер вернул пустой ответ."));return}const n=o.body.getReader(),i=new TextDecoder;let d="";try{for(;;){const{done:l,value:m}=await n.read();if(l)break;d+=i.decode(m,{stream:!0});const w=d.split(`
`);d=w.pop()||"";for(const h of w){const u=h.trim();if(!u.startsWith("data:"))continue;const b=u.slice(5).trim();if(b){if(b==="[DONE]"){t.onDone();return}try{const g=JSON.parse(b);g.type==="delta"&&g.delta?t.onDelta(g.delta):g.type==="error"?t.onError(g.error||S("Assistant error.","Ошибка помощника.")):g.type}catch{}}}}t.onDone()}catch(l){if(l?.name==="AbortError"){t.onDone();return}t.onError(S("Stream interrupted. Please try again.","Ответ прервался. Попробуйте ещё раз."))}}const ne=3500;function ie(){const e=document.title||"",t=location.href,o=(document.querySelector("main")||document.querySelector("article")||document.querySelector(".review-content")||document.querySelector(".page-content")||document.body).cloneNode(!0);o.querySelectorAll('script, style, noscript, nav, header, footer, .site-header, .site-footer, [id="1winex-chat-root"], [aria-hidden="true"]').forEach(i=>i.remove());const n=(o.textContent||"").replace(/\s+/g," ").trim().slice(0,ne);return{title:e,url:t,snippet:n}}function se(e){const t='a[href], button:not([disabled]), textarea:not([disabled]), input:not([disabled]), select:not([disabled]), [tabindex]:not([tabindex="-1"])';function a(){return Array.from(e.querySelectorAll(t)).filter(n=>!n.hasAttribute("disabled")&&n.offsetParent!==null)}function o(n){if(n.key!=="Tab")return;const i=a();if(i.length===0){n.preventDefault();return}const d=i[0],l=i[i.length-1];n.shiftKey?document.activeElement===d&&(n.preventDefault(),l.focus()):document.activeElement===l&&(n.preventDefault(),d.focus())}return{activate(){e.addEventListener("keydown",o),(a()[0]||e).focus()},deactivate(){e.removeEventListener("keydown",o)}}}const le="modulepreload",ce=function(e,t){return new URL(e,t).href},W={},x=function(t,a,o){let n=Promise.resolve();if(a&&a.length>0){const d=document.getElementsByTagName("link"),l=document.querySelector("meta[property=csp-nonce]"),m=l?.nonce||l?.getAttribute("nonce");n=Promise.allSettled(a.map(w=>{if(w=ce(w,o),w in W)return;W[w]=!0;const h=w.endsWith(".css"),u=h?'[rel="stylesheet"]':"";if(!!o)for(let y=d.length-1;y>=0;y--){const v=d[y];if(v.href===w&&(!h||v.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${w}"]${u}`))return;const g=document.createElement("link");if(g.rel=h?"stylesheet":le,h||(g.as="script"),g.crossOrigin="",g.href=w,m&&g.setAttribute("nonce",m),document.head.appendChild(g),h)return new Promise((y,v)=>{g.addEventListener("load",y),g.addEventListener("error",()=>v(new Error(`Unable to preload CSS for ${w}`)))})}))}function i(d){const l=new Event("vite:preloadError",{cancelable:!0});if(l.payload=d,window.dispatchEvent(l),!l.defaultPrevented)throw d}return n.then(d=>{for(const l of d||[])l.status==="rejected"&&i(l.reason);return t().catch(i)})};let N=null,H=null,O=null;async function de(){return(await x(()=>import("./chat-widget-core-Cy0t5JvF.js"),[],import.meta.url)).default}async function ue(){if(!N||!H){const[e,t]=await Promise.all([x(()=>import("./chat-widget-marked.esm-DweX3G3F.js"),[],import.meta.url),x(()=>import("./chat-widget-purify.es-BnINGy_Y.js"),[],import.meta.url)]);N=e,H=t,e.marked.setOptions({gfm:!0,breaks:!0})}return{marked:N.marked,DOMPurify:H.default}}async function pe(){if(O)return O;const e=await de(),t=await Promise.all([x(()=>import("./chat-widget-javascript-DUNaicwC.js"),[],import.meta.url),x(()=>import("./chat-widget-typescript-5pldp2kO.js"),[],import.meta.url),x(()=>import("./chat-widget-json-DdsocbVI.js"),[],import.meta.url),x(()=>import("./chat-widget-bash-I8pq0VWm.js"),[],import.meta.url),x(()=>import("./chat-widget-xml-FcirFJJ2.js"),[],import.meta.url),x(()=>import("./chat-widget-css-AVCICof-.js"),[],import.meta.url),x(()=>import("./chat-widget-python-C7NRPp2R.js"),[],import.meta.url)]);return["javascript","typescript","json","bash","xml","css","python"].forEach((o,n)=>e.registerLanguage(o,t[n].default)),O=e,e}const j={ALLOWED_TAGS:["p","br","strong","em","u","s","code","pre","blockquote","ul","ol","li","a","h1","h2","h3","h4","table","thead","tbody","tr","th","td","hr","span","button","div"],ALLOWED_ATTR:["href","title","target","rel","class","data-lang","type","aria-label"]},ge="nofollow noopener noreferrer",me=new Set(["1winex.com","www.1winex.com"]);function Y(e){return!(!e||e.startsWith("#")||/^(mailto|tel|javascript|data):/i.test(e))}function he(e){try{const t=typeof location<"u"?location.href:"https://1winex.com/",a=new URL(e,t).pathname.replace(/\/+$/,"")||"/";return a==="/go"||a==="/apk"}catch{return!1}}function we(e){const t=e.trim();if(!Y(t)||he(t))return!1;if(!/^https?:\/\//i.test(t)&&!t.startsWith("//"))return!/^[a-z][a-z0-9+.-]*:/i.test(t);try{const a=t.startsWith("//")?`https:${t}`:t,o=new URL(a).hostname.toLowerCase();if(me.has(o)||typeof location<"u"&&location.hostname&&o===location.hostname.toLowerCase())return!0}catch{return!1}return!1}function K(e){e.querySelectorAll("a[href]").forEach(t=>{const a=t,o=(a.getAttribute("href")||"").trim();if(!Y(o))return;const n=/^https?:\/\//i.test(o)||o.startsWith("//"),i=!n&&!/^[a-z][a-z0-9+.-]*:/i.test(o);if(!(!n&&!i)){if(we(o)){a.removeAttribute("target"),a.removeAttribute("rel");return}a.setAttribute("target","_blank"),a.setAttribute("rel",ge)}})}function fe(e){const t=document.createElement("div");return t.innerHTML=e,K(t),t.innerHTML}async function be(e){const{marked:t,DOMPurify:a}=await ue(),o=await t.parse(e||"");let n=a.sanitize(String(o),j);if(/<pre[\s>]/i.test(n)){const i=await pe(),d=document.createElement("div");d.innerHTML=n,d.querySelectorAll("pre code").forEach(l=>{const m=l,h=(m.className||"").match(/language-([\w-]+)/)?.[1];try{h&&i.getLanguage(h)?m.innerHTML=i.highlight(m.textContent||"",{language:h}).value:m.innerHTML=i.highlightAuto(m.textContent||"").value}catch{}const u=m.parentElement;if(u&&u.tagName==="PRE"&&!u.querySelector(".aw-code-copy")){u.setAttribute("data-lang",h||"code");const b=document.createElement("button");b.type="button",b.className="aw-code-copy";const g=(document.documentElement.lang||"").toLowerCase().startsWith("ru");b.setAttribute("aria-label",g?"Копировать код":"Copy code"),b.textContent=g?"Копировать":"Copy",u.appendChild(b)}}),n=a.sanitize(d.innerHTML,j)}return fe(n)}function V(){try{const e=localStorage.getItem(U);if(!e)return null;const t=JSON.parse(e);return!t||!Array.isArray(t.messages)?null:t}catch{return null}}function _e(e){try{localStorage.setItem(U,JSON.stringify(e))}catch{}}function ve(){const e=V();return e?e.messages.slice(-60):[]}function xe(){return!!V()?.unread}function ye(e,t=!1){_e({messages:e.slice(-60),updatedAt:Date.now(),unread:!!t})}function Ee(){try{const e=localStorage.getItem(ee);if(e==="light"||e==="dark")return e}catch{}return"dark"}function ke(){return typeof document>"u"?!1:(document.documentElement.lang||"").toLowerCase().startsWith("ru")}const Ae=["What license does 1win have?","How does the 600% bonus work?","How fast are withdrawals?","Does 1win have a mobile app?"],Se=["Какая лицензия у 1win?","Как работает бонус 600%?","Как быстро выводят деньги?","Есть ли у 1win приложение?"];function _(e,t=4){const a=[],o=new Set;for(const n of e){const i=String(n||"").replace(/\s+/g," ").trim();if(!i)continue;const d=i.toLowerCase();if(!o.has(d)&&(o.add(d),a.push(i),a.length>=t))break}return a}function Le(e=location.pathname){const a=(e.replace(/\/+$/,"")||"/").split("/").filter(Boolean);if(!a.length)return"index";let o=0;if(a[0]==="ru"&&(o=1),!a[o]||a[o]==="index")return"index";let n=a[o].replace(/\.html$/i,"");return n==="betting"?"sports":n==="app"?"mobile":n==="casino"?"games":n}function Te(e=typeof location<"u"?location.pathname:"/"){const t=Le(e),a=ke();return _(t==="bonuses"?a?["Как работает бонус 600%?","Какие условия отыгрыша?","Куда вводить WINEX600?","Как получить приветственный бонус?"]:["How does the 600% bonus work?","What are the wagering requirements?","Where do I enter WINEX600?","How do I claim the welcome package?"]:t==="payments"?a?["Как быстро выводят деньги?","Поддерживает ли 1win криптовалюту?","Какая минимальная сумма вывода?","Сколько длится проверка KYC?"]:["How fast are withdrawals?","Does 1win support crypto?","What is the minimum withdrawal?","How long does KYC take?"]:t==="mobile"?a?["Как установить Android APK?","Есть ли приложение для iOS?","Можно ли играть в мобильном браузере?","Есть ли у 1win приложение?"]:["How do I install the Android APK?","Is there an iOS app?","Can I play in the mobile browser?","Does 1win have a mobile app?"]:t==="games"?a?["Какие слоты есть в 1win?","Какой RTP у Aviator?","Есть ли в 1win live-казино?","Как работает бонус 600%?"]:["What slots does 1win have?","What is Aviator RTP?","Does 1win have live casino?","How does the 600% bonus work?"]:t==="aviator"?a?["Какой RTP у Aviator?","Aviator — это 1win Original?","Идёт ли Aviator в отыгрыш бонуса?","Что такое автокэшаут в Aviator?"]:["What is Aviator RTP?","Is Aviator a 1win Original?","Does Aviator count toward bonus wagering?","What is auto cash-out in Aviator?"]:t==="lucky-jet"?a?["Какой RTP у Lucky Jet?","Lucky Jet — это то же самое, что Aviator?","Идёт ли Lucky Jet в отыгрыш бонуса?","Что такое автокэшаут в Lucky Jet?"]:["What is Lucky Jet RTP?","Is Lucky Jet the same as Aviator?","Does Lucky Jet count toward bonus wagering?","What is auto cash-out in Lucky Jet?"]:t==="sports"?a?["Как сделать ставку на спорт в 1win?","Есть ли кэшаут ставок?","Что такое бустер на экспресс?","У бонусов казино и спорта общий баланс?"]:["How do I place a 1win sports bet?","Can I cash out a sports bet?","What is Express booster?","Do casino and sports bonuses share a balance?"]:t==="safety"?a?["Какая лицензия у 1win?","Сколько длится проверка KYC?","Надёжно ли хранятся средства в 1win?","Есть ли в 1win 2FA?"]:["What license does 1win have?","How long does KYC take?","Is my money safe at 1win?","Does 1win have 2FA?"]:t==="faq"?a?["Как зарегистрироваться в 1win?","Как быстро выводят деньги?","Как работает бонус 600%?","Есть ли у 1win приложение?"]:["How do I register at 1win?","How fast are withdrawals?","How does the 600% bonus work?","Does 1win have a mobile app?"]:t==="responsible-gambling"?a?["Какие есть лимиты и самоисключение в 1win?","Как установить лимит на депозит?","Как работает самоисключение?","Какая лицензия у 1win?"]:["What responsible gambling tools does 1win offer?","How do I set a deposit limit?","How does self-exclusion work?","What license does 1win have?"]:[...a?Se:Ae])}const Ce=`
.aw-chat {
...
`;function Ie(){if(document.getElementById("1winex-chat-styles"))return;const e=document.createElement("style");e.id="1winex-chat-styles",e.textContent=Ce,document.head.appendChild(e)}function z(){return`m_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,8)}`}function De(e){try{return new Intl.DateTimeFormat(void 0,{hour:"numeric",minute:"2-digit"}).format(new Date(e))}catch{return""}}const G={chat:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>',close:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" aria-hidden="true"><path d="M18 6 6 18M6 6l12 12"/></svg>'},Me=new URL("../images/chat/anna.webp?v=3",import.meta.url).href;function B(e){return e.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Re(){return(document.documentElement.lang||"").toLowerCase().startsWith("ru")}function Pe(){return Re()?{name:"Анна",subtitle:"Виртуальный помощник 1win",close:"Закрыть чат",placeholder:"Задайте вопрос…",message:"Сообщение",send:"Отправить",sendMessage:"Отправить сообщение",open:"Открыть чат с Анной",closeAnna:"Закрыть чат с Анной",openUnread:"Открыть чат с Анной — новое сообщение",typing:"Анна печатает",hello:"Привет! Я Анна. Чем могу помочь?",intro:"Могу подсказать по казино, ставкам на спорт, промокоду WINEX600, депозитам, выводам и Android APK.",suggests:"Популярные вопросы",noResponse:"Не удалось получить ответ.",copied:"Скопировано",copy:"Копировать",failed:"Ошибка"}:{name:"Anna",subtitle:"Virtual 1win assistant",close:"Close chat",placeholder:"Ask a question…",message:"Message",send:"Send",sendMessage:"Send message",open:"Open chat with Anna",closeAnna:"Close chat with Anna",openUnread:"Open chat with Anna — new message",typing:"Anna is typing",hello:"Hi! I&rsquo;m Anna. How can I help?",intro:"I can help with casino and sports betting, promo code WINEX600, deposits, withdrawals, and the Android APK.",suggests:"Suggested questions",noResponse:"No response received.",copied:"Copied",copy:"Copy",failed:"Failed"}}function Ne(){if(document.getElementById($))return;Ie();const e=Pe();let t=ve(),a=!1,o=null,n=null;const i=document.createElement("div");i.id=$,i.className="aw-chat",i.dataset.theme=Ee(),i.setAttribute("data-nosnippet",""),i.innerHTML=`
  ...
  `,document.body.appendChild(i);const d=i.querySelector(".aw-chat__panel");d.id="aw-chat-panel";const l=i.querySelector(".aw-chat__messages"),m=i.querySelector(".aw-chat__composer"),w=m.querySelector("textarea"),h=m.querySelector(".aw-chat__send"),u=i.querySelector(".aw-chat__toggle"),b=u.querySelector(".aw-chat__badge"),g=se(d);let y=null;const v=new Map;let E=0,L=xe();function k(){ye(t.filter(r=>r.status!=="streaming"),L)}function F(r){l.querySelector(`[data-id="${r}"]`)?.scrollIntoView({block:"start",behavior:"smooth"})}function I(){const r=L&&!a;i.classList.toggle("has-unread",r),b.hidden=!r,b.setAttribute("aria-hidden",r?"false":"true"),a?u.setAttribute("aria-label",e.closeAnna):r?u.setAttribute("aria-label",e.openUnread):u.setAttribute("aria-label",e.open)}function D(){a||(L=!0,k(),I())}function T(r){a=r,i.classList.toggle("is-open",a),u.setAttribute("aria-expanded",String(a)),d.hidden=!a,d.setAttribute("aria-hidden",a?"false":"true"),a?(L=!1,k(),y=document.activeElement,g.activate()):(g.deactivate(),(y||u).focus()),I()}async function C(r,s){const p=(v.get(r.id)??0)+1;v.set(r.id,p);let f=s;f||(f=document.createElement("div"),f.className=`aw-msg aw-msg--${r.role}`,f.dataset.id=r.id,l.appendChild(f));const c=document.createElement("div");if(c.className="aw-msg__bubble",r.role==="assistant")if(r.status==="streaming"&&!r.content)c.innerHTML=`<span class="aw-typing" aria-label="${e.typing}"><i></i><i></i><i></i></span>`;else try{const P=await be(r.content||"");if(v.get(r.id)!==p)return;c.innerHTML=P,K(c)}catch{if(v.get(r.id)!==p)return;c.textContent=r.content||""}else c.textContent=r.content;if(v.get(r.id)!==p)return;const A=document.createElement("div");A.className="aw-msg__meta",A.textContent=De(r.createdAt),f.replaceChildren(c,A)}async function J(r){const s=l.scrollTop;if(l.replaceChildren(),t.length===0){const p=document.createElement("div");p.className="aw-chat__empty";const f=Te().map(c=>`<button type="button" class="aw-chat__suggest" data-suggest="${B(c)}">${B(c)}</button>`).join("");p.innerHTML=`
        <strong>${e.hello}</strong>
        <p>${e.intro}</p>
        <div class="aw-chat__suggests" role="group" aria-label="${e.suggests}">${f}</div>
      `,l.appendChild(p);return}for(const p of t)await C(p);l.scrollTop=s}function M(r){h.className="aw-chat__send",h.textContent=e.send,h.type="submit",h.disabled=r,h.setAttribute("aria-label",e.sendMessage),h.onclick=null}function R(r){E&&(cancelAnimationFrame(E),E=0);const s=l.querySelector(`[data-id="${r.id}"]`);return C(r,s||void 0)}function X(r){E||(E=requestAnimationFrame(()=>{E=0,R(r)}))}async function Q(r){const s={id:z(),role:"assistant",content:"",createdAt:Date.now(),status:"streaming"};t=[...r,s],n=s.id,M(!0),await C(s);const p=[...r].reverse().find(c=>c.role==="user");p&&F(p.id),o=new AbortController;const f={messages:r.filter(c=>c.role==="user"||c.role==="assistant").filter(c=>c.status!=="error").map(c=>({role:c.role,content:c.content.slice(0,2e3)})),pageContext:ie(),site:ae};await oe(f,{onDelta:c=>{s.content+=c,s.status="streaming",X(s),s.content&&D()},onDone:()=>{const c=!!s.content;s.status=c?"ok":"error",c||(s.content=e.noResponse),n=null,o=null,M(!1),k(),R(s),D()},onError:c=>{s.content=c,s.status="error",n=null,o=null,M(!1),k(),R(s),D()}},o.signal)}async function q(r){const s=r.trim().slice(0,2e3);if(!s||n)return;const p=l.querySelector(".aw-chat__empty");p&&p.remove();const f={id:z(),role:"user",content:s,createdAt:Date.now(),status:"ok"},c=[...t,f];t=c,k(),await C(f),await Q(c)}function Z(){window.matchMedia("(prefers-reduced-motion: reduce)").matches||(u.classList.remove("aw-chat__toggle--spin","aw-chat__toggle--enter"),u.offsetWidth,u.classList.add("aw-chat__toggle--spin"))}window.matchMedia("(prefers-reduced-motion: reduce)").matches||u.classList.add("aw-chat__toggle--enter"),u.addEventListener("animationend",r=>{r.animationName==="aw-toggle-spin"&&u.classList.remove("aw-chat__toggle--spin"),r.animationName==="aw-toggle-enter"&&u.classList.remove("aw-chat__toggle--enter")}),u.addEventListener("click",()=>{Z(),T(!a)}),i.querySelector('[data-action="close"]').addEventListener("click",()=>T(!1)),m.addEventListener("submit",r=>{r.preventDefault();const s=w.value;w.value="",q(s)}),w.addEventListener("keydown",r=>{r.key==="Enter"&&!r.shiftKey&&(r.preventDefault(),m.requestSubmit())}),l.addEventListener("click",async r=>{const s=r.target.closest(".aw-chat__suggest");if(s){const P=s.getAttribute("data-suggest")||s.textContent||"";q(P);return}const p=r.target.closest(".aw-code-copy");if(!p)return;const A=p.closest("pre")?.querySelector("code")?.textContent||"";try{await navigator.clipboard.writeText(A),p.textContent=e.copied,setTimeout(()=>{p.textContent=e.copy},1200)}catch{p.textContent=e.failed}}),document.addEventListener("keydown",r=>{r.key==="Escape"&&a&&(r.preventDefault(),T(!1))}),document.addEventListener("pointerdown",r=>{if(!a||n)return;const s=r.target;s&&(d.contains(s)||u.contains(s)||T(!1))},!0),J(),I()}Ne();
